...
import {
    ...
    DialogTitle,
    DialogTrigger,
} from "@/components/ui/dialog"; // Asegúrate de que la ruta a tus componentes de shadcn sea correcta
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogTrigger asChild>
                <button
                    className={`w-sm text-center text-7xl text-neutral-400 p-4 my-${my ?? 5} hover:text-neutral-100 hover:scale-110 transition duration-500 ease-in-out`}
                >
                    {buttonText}
                </button>
            </DialogTrigger>
            ...
            <DialogContent className="bg-neutral-900/95 backdrop-blur-md border-neutral-700 text-white max-w-md">
                <DialogHeader>
                    <DialogTitle className="text-4xl text-neutral-200 text-center mb-4">
                        {dialogTitle}
                    </DialogTitle>
                </DialogHeader>
                ...
                <div className="flex flex-col items-center justify-center space-y-2 py-4">
                    ...
                    {urlItems && urlItems.map((urlItem, index) => (
                        //   {items.map((item, index) => (
                        <Link
                            key={index}
                            href={urlItem.href}
                            className="w-full text-center text-3xl text-neutral-300 p-2 hover:text-white transition duration-300"
